    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-danger">
                <h5 class="modal-work_name text-white delete_modal_text"></h5>
            </div>
    ...
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header bg-primary">
                <h5 class="modal-work_name text-white" id="myExtraLargeModalLabel">Add / Update Work</h5>
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-lg-4 mb-3">
                            <label for="work_name" class="form-label">Work Title<span class="required-field">*</span></label>
                            <input type="text" class="form-control" id="work_name" name="work_name" placeholder="Enter Work Title">
                        </div>
                        <div class="col-lg-4 mb-3">
                            <label for="work_desc" class="form-label">Work Description</label>
                            <textarea class="form-control" id="work_desc" name="work_desc" placeholder="Enter Work Description"></textarea>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
...
var table = $('#datatable').DataTable({
    processing: true,
    serverSide: true,
    ajax: "{{ route('mst_works.getMstWorksRecords') }}",
    columns: [
        {data: 'action', name: 'action', orderable: false, searchable: false},
        {data: 'work_name', name: 'work_name'},
        {data: 'work_desc', name: 'work_desc'},
        {data: 'work_updatedon', name: 'work_updatedon'}
    ]
});
...
$('#datatable').on('click', '.edit', function() {
    var id = $(this).data('id');
    clearForm('add_update_form');
    var url = "{{ route('mst_works.getMstWorks', ':id') }}";
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#add_update_modal').modal('show');
            $('#add_update_modal #work_name').val(response.data.work_name);
            $('#add_update_modal #work_desc').val(response.data.work_desc);
            $('#add_update_modal #work_id').val(response.data.work_id);
        },
        error: function(xhr) {
            if (xhr.status === 404) {
...
$(document).on('click', '.delete_records', function () {
    ...
    var url = "{{ route('mst_works.delete') }}";
    $.ajax({
        url: url,
        type: 'POST',
        headers: {
